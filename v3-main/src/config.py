...
class Settings(BaseSettings):
    ...
    anthropic_model: str = Field(
        default="claude-3-5-sonnet-20240620", description="Anthropic model for generation"
    )
    voyage_embed_model: str = Field(
        default="voyage-large-2-instruct", description="Voyage model for embeddings"
    )
    cohere_rerank_model: str = Field(
        default="rerank-english-v3.0", description="Cohere model for re-ranking"
    )
    embedding_dimension: int = Field(default=1024, description="Dimension of the embedding vectors")
    ...
    cdk_client_secret: str = Field(default="", description="CDK Global client secret")
    cdk_dealer_id: str = Field(default="", description="CDK Global dealer ID")
    cdk_api_url: str = Field(default="https://api.fortellis.io", description="CDK API URL")
    
    reynolds_api_key: str = Field(default="", description="Reynolds & Reynolds API key")
    reynolds_dealer_code: str = Field(default="", description="Reynolds dealer code")
    reynolds_mfa_token: str = Field(default="", description="Reynolds MFA token")
    reynolds_api_url: str = Field(
        default="https://api.reyrey.com/v2", description="Reynolds API URL"
    )
    
    dms_adapter: Literal["cdk", "reynolds", "mock"] = Field(
        default="mock", description="DMS adapter to use"
    )
    ...
    elasticsearch_username: str = Field(default="", description="Elasticsearch username")
    ...
    elasticsearch_verify_certs: bool = Field(default=True, description="Verify SSL certificates")
    elasticsearch_cloud_id: str = Field(default="", description="Elasticsearch Cloud ID")
    ...
    api_secret_key: str = Field(
        default="dev-secret-change-in-production", description="Secret key for API authentication"
    )
    ...
    otlp_endpoint: str = Field(default="", description="OTLP endpoint for traces and metrics")
    otlp_metrics_endpoint: str = Field(default="", description="OTLP endpoint specifically for metrics")
    otlp_token: str = Field(default="", description="Authentication token for OTLP")
    otlp_secure: bool = Field(default=True, description="Use secure connection for OTLP")
    prometheus_port: int = Field(default=8001, description="Port for Prometheus metrics endpoint")
    jaeger_endpoint: str = Field(default="", description="Jaeger endpoint for tracing")
...
async def validate_api_keys_at_startup():
    ...
    if settings.voyage_api_key:
        try:
            ...
            await asyncio.to_thread(client.embed, ["test"], model=settings.voyage_embed_model)
